...
import { isSlotTimedOut, todayKeyDhaka } from '../../utils/slots.js';
    ...
    <div className="space-y-5">
      ...
      {isLoading ? (
        ...
        <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5">
          {slots.map((s) => {
            const timedOut = isSlotTimedOut(s);
            return (
            <Card key={s._id} className={`p-3 ${timedOut ? 'opacity-60' : ''}`}>
              <div className="flex items-center justify-between">
                <span className="font-semibold text-white">{fmtTime(s.startTime)}</span>
                <Badge tone={timedOut ? 'gray' : s.status === 'available' ? 'green' : s.status === 'booked' ? 'red' : 'gray'}>
                  {timedOut ? 'timed out' : s.status}
                </Badge>
              </div>
              <p className="mt-1 text-xs text-ink-500">{fmtTime(s.endTime)} · {taka(s.price)}</p>
              {s.status !== 'booked' && !timedOut && (
                <button
              ...
              {timedOut && s.status !== 'booked' && (
                <p className="mt-2 text-center text-[10px] uppercase text-ink-600">Slot ended</p>
              )}
            </Card>
            );
          })}
